...
@MainActor
public func handlePerform(cmd: CommandEnvelope, isDebugLoggingEnabled: Bool) throws -> PerformResponse {
    ...
    dLog("Handling perform_action for app: \(cmd.application ?? focusedApplicationKey), action: \(cmd.action ?? "nil")")
    ...
    guard let appElement = applicationElement(for: cmd.application ?? focusedApplicationKey, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) else {
        // If applicationElement itself logged to a global store, that won't be in handlerLogs.
        // For now, this is acceptable as an intermediate step.
        return PerformResponse(command_id: cmd.command_id, success: false, error: "Application not found: \(cmd.application ?? focusedApplicationKey)", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
    }
    guard let actionToPerform = cmd.action, !actionToPerform.isEmpty else {
        return PerformResponse(command_id: cmd.command_id, success: false, error: "Action not specified", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
    }
    guard let locator = cmd.locator else {
        ...
        if let pathHint = cmd.path_hint, !pathHint.isEmpty {
            ...
            guard let navigatedElement = navigateToElement(from: appElement, pathHint: pathHint, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) else {
                return PerformResponse(command_id: cmd.command_id, success: false, error: "Element for action (no locator) not found via path_hint: \(pathHint.joined(separator: " -> "))", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
            }
            elementForDirectAction = navigatedElement
        }
        let briefDesc = elementForDirectAction.briefDescription(option: .default, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs)
        dLog("No locator. Performing action '\(actionToPerform)' directly on element: \(briefDesc)")
        // performActionOnElement is a private helper in this file, so it CAN use handlerLogs.
        return try performActionOnElement(element: elementForDirectAction, action: actionToPerform, cmd: cmd, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs)
    }
    ...
    if let pathHint = cmd.path_hint, !pathHint.isEmpty {
        ...
        guard let navigatedBase = navigateToElement(from: appElement, pathHint: pathHint, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) else {
            return PerformResponse(command_id: cmd.command_id, success: false, error: "Base element for search (from main path_hint) not found: \(pathHint.joined(separator: " -> "))", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
        }
    ...
    if let rootPathHint = locator.root_element_path_hint, !rootPathHint.isEmpty {
        ...
        guard let newBaseFromLocatorRoot = navigateToElement(from: appElement, pathHint: rootPathHint, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) else {
            return PerformResponse(command_id: cmd.command_id, success: false, error: "Search base from locator.root_element_path_hint not found: \(rootPathHint.joined(separator: " -> "))", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
        }
        baseElementForSearch = newBaseFromLocatorRoot
    }
    let baseBriefDesc = baseElementForSearch.briefDescription(option: .default, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs)
    ...
    // search() is external, call original signature. Its logs won't be in handlerLogs yet.
    var targetElement: Element? = search(element: baseElementForSearch, locator: locator, requireAction: actionRequiredForInitialSearch, maxDepth: cmd.max_elements ?? DEFAULT_MAX_DEPTH_SEARCH, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs)

    if targetElement == nil || 
       (actionToPerform != kAXSetValueAction && 
        actionToPerform != kAXPressAction && 
        targetElement?.isActionSupported(actionToPerform, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) == false) {
        ...
        if let titleFromCriteria = smartLocatorCriteria[kAXTitleAttribute] ?? smartLocatorCriteria[kAXTitleAttribute] {
            smartLocatorCriteria[computedNameAttributeKey + "_contains"] = titleFromCriteria
        ...
        } else if let idFromCriteria = smartLocatorCriteria[kAXIdentifierAttribute] ?? smartLocatorCriteria[kAXIdentifierAttribute] {
            smartLocatorCriteria[computedNameAttributeKey + "_contains"] = idFromCriteria
        ...
        if useComputedNameForSmartSearch || (smartLocatorCriteria[kAXRoleAttribute] != nil) {
            let smartSearchLocator = Locator(
                ...
                computed_name_contains: smartLocatorCriteria[computedNameAttributeKey + "_contains"]
            )
            ...
            // collectAll() is external, call original signature. Its logs won't be in handlerLogs yet.
            collectAll(
                ...
                isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs
            )
            let trulySupportingElements = foundCollectedElements.filter { $0.isActionSupported(actionToPerform, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) }
            if trulySupportingElements.count == 1 {
                targetElement = trulySupportingElements.first
                let targetDesc = targetElement?.briefDescription(option: .verbose, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) ?? "nil"
    ...
    guard let finalTargetElement = targetElement else {
        return PerformResponse(command_id: cmd.command_id, success: false, error: "Target element for action '\(actionToPerform)' not found, even after smart search.", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
    }
    
    if actionToPerform != kAXSetValueAction && !finalTargetElement.isActionSupported(actionToPerform, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs) {
         let supportedActions: [String]? = finalTargetElement.supportedActions(isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs)
         return PerformResponse(command_id: cmd.command_id, success: false, error: "Final target element for action '\(actionToPerform)' does not support it. Supported: \(supportedActions?.joined(separator: ", ") ?? "none")", debug_logs: isDebugLoggingEnabled ? handlerLogs : nil)
    }

    return try performActionOnElement(element: finalTargetElement, action: actionToPerform, cmd: cmd, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &handlerLogs)
}
...
private func performActionOnElement(element: Element, action: String, cmd: CommandEnvelope, isDebugLoggingEnabled: Bool, currentDebugLogs: inout [String]) throws -> PerformResponse {
    ...
    if action == kAXSetValueAction {
        guard let valueToSetString = cmd.value else {
            return PerformResponse(command_id: cmd.command_id, success: false, error: "Value not provided for AXSetValue action", debug_logs: isDebugLoggingEnabled ? currentDebugLogs : nil)
        }
        ...
        do {
            ...
            guard let cfValueToSet = try createCFTypeRefFromString(stringValue: valueToSetString, forElement: element, attributeName: attributeToSet, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &currentDebugLogs) else {
                 return PerformResponse(command_id: cmd.command_id, success: false, error: "Could not parse value '\(valueToSetString)' for attribute '\(attributeToSet)'. Parsing returned nil.", debug_logs: isDebugLoggingEnabled ? currentDebugLogs : nil)
            }
            let axErr = AXUIElementSetAttributeValue(element.underlyingElement, attributeToSet as CFString, cfValueToSet)
            if axErr == .success {
                return PerformResponse(command_id: cmd.command_id, success: true, error: nil, debug_logs: isDebugLoggingEnabled ? currentDebugLogs : nil)
            } else {
        ...
        if !element.isActionSupported(action, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &currentDebugLogs) {
            if action == kAXPressAction && cmd.perform_action_on_child_if_needed == true {
                ...
                if let children = element.children(isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &currentDebugLogs), !children.isEmpty {
                    ...
                    return PerformResponse(command_id: cmd.command_id, success: false, error: "Action '\(action)' not supported, and no children to attempt alternative press.", debug_logs: isDebugLoggingEnabled ? currentDebugLogs : nil)
                } else {
            ...
            return PerformResponse(command_id: cmd.command_id, success: false, error: "Action '\(action)' not supported. Supported: \(supportedActions?.joined(separator: ", ") ?? "none")", debug_logs: isDebugLoggingEnabled ? currentDebugLogs : nil)
        }
        do {
