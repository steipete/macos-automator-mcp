...
public func handlePerform(cmd: CommandEnvelope, isDebugLoggingEnabled: Bool, currentDebugLogs: inout [String]) throws -> PerformResponse {
    ...
    guard let appElement = applicationElement(for: cmd.application ?? focusedApplicationKey, isDebugLoggingEnabled: isDebugLoggingEnabled, currentDebugLogs: &currentDebugLogs) else {
       ...
       (actionToPerform != kAXSetValueAction && 
        ...
        if useComputedNameForSmartSearch || (smartLocatorCriteria[kAXRoleAttribute] != nil) {
            let smartSearchLocator = Locator(
                ...
                computed_name_equals: nil, computed_name_contains: smartLocatorCriteria[computedNameAttributeKey + "_contains"]
            )
