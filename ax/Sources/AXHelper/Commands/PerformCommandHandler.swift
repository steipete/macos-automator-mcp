...
func handlePerform(cmd: CommandEnvelope, isDebugLoggingEnabled: Bool) throws -> PerformResponse {
    ...
    debug("PerformAction: Searching for action element within: \(baseElementForSearch.underlyingElement) using locator criteria: \(locator.criteria)")
    ...
    var targetElement: Element? = search(element: baseElementForSearch, locator: locator, requireAction: actionRequiredForInitialSearch, maxDepth: cmd.max_elements ?? DEFAULT_MAX_DEPTH_SEARCH, isDebugLoggingEnabled: isDebugLoggingEnabled)

    // Smart Search / Fuzzy Find for perform_action
    if targetElement == nil || 
       (actionToPerform != kAXSetValueAction && 
        actionToPerform != kAXPressAction && 
        targetElement?.isActionSupported(actionToPerform) == false) {
        ...
        if useComputedNameForSmartSearch || (smartLocatorCriteria[kAXRoleAttribute] != nil || smartLocatorCriteria["AXRole"] != nil) {
            ...
            collectAll(
                ...
                currentElement: baseElementForSearch, 
                depth: 0, 
            ...
            if trulySupportingElements.count == 1 {
                ...
                debug("PerformAction (Smart): Found unique element via smart search: \(targetElement?.briefDescription(option: .verbose) ?? "nil")")
    ...
    if actionToPerform != kAXSetValueAction && !finalTargetElement.isActionSupported(actionToPerform) {
         let supportedActions: [String]? = finalTargetElement.supportedActions
    ...
    return try performActionOnElement(element: finalTargetElement, action: actionToPerform, cmd: cmd)
}

@MainActor
private func performActionOnElement(element: Element, action: String, cmd: CommandEnvelope) throws -> PerformResponse {
    debug("Final target element for action '\(action)': \(element.underlyingElement)")
    if action == kAXSetValueAction {
        ...
        debug("AXSetValue: Attempting to set attribute '\(attributeToSet)' to value '\(valueToSetString)' on \(String(describing: element.underlyingElement))")

        do {
            guard let cfValueToSet = try createCFTypeRefFromString(stringValue: valueToSetString, forElement: element, attributeName: attributeToSet) else {
        ...
        if !element.isActionSupported(action) {
            if action == kAXPressAction && cmd.perform_action_on_child_if_needed == true {
                debug("Action '\(action)' not supported on element \(element.briefDescription()). Trying on children as perform_action_on_child_if_needed is true.")
                    ...
                    for child in children {
                        if child.isActionSupported(kAXPressAction) {
                            ...
                            do {
                                ...
                                return PerformResponse(command_id: cmd.command_id, success: true, error: nil, debug_logs: collectedDebugLogs)
                            } catch AccessibilityError.actionFailed(_, _) {
            ...
            let supportedActions: [String]? = element.supportedActions
        ...
        debug("Performing action '\(action)' on \(element.underlyingElement)")
