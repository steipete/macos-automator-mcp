...
// Constants are assumed to be globally available from AXConstants.swift
// AXScanner and AXCharacterSet are from AXScanner.swift
// AXToolError is from AXError.swift

// Inspired by UIElementInspector's UIElementUtilities.m

// AXValueParseError enum has been removed and its cases merged into AXToolError.

@MainActor
public func getCFTypeIDForAttribute(element: AXElement, attributeName: String) -> CFTypeID? {
...
@MainActor
public func getAXValueTypeForAttribute(element: AXElement, attributeName: String) -> AXValueType? {
    guard let rawValue = element.rawAttributeValue(named: attributeName) else {
...
@MainActor
public func createCFTypeRefFromString(stringValue: String, forElement element: AXElement, attributeName: String) throws -> CFTypeRef? {
    guard let currentRawValue = element.rawAttributeValue(named: attributeName) else {
        throw AXToolError.attributeNotReadable("Could not read current value for attribute '\(attributeName)' to determine type.")
    }
    ...
    if typeID == AXValueGetTypeID() {
        ...
        if let doubleValue = Double(stringValue) {
            ...
            throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' as Double or Int for CFNumber attribute '\(attributeName)'")
        }
        ...
        if stringValue.lowercased() == "true" {
            ...
            throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' as Bool (true/false) for CFBoolean attribute '\(attributeName)'")
        }
    }
    ...
    throw AXToolError.attributeUnsupported("Setting attribute '\(attributeName)' of CFTypeID \(typeID) (\(typeDescription)) from string is not supported yet.")
}
...
private func parseStringToAXValue(stringValue: String, targetAXValueType: AXValueType) throws -> AXValue? {
    ...
    case .cgPoint:
        ...
        // Alternative parsing for formats like "x:10 y:20"
             ...
             let scanner = AXScanner(string: stringValue)
             ...
             if let xVal = xScanned, let yVal = yScanned {
                ...
                throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' into CGPoint. Expected format like 'x=10,y=20' or '10,20'.")
             }
        }
           ...
           let hValStr = components[1].split(separator: "=").last, let hVal = Double(hValStr) {
            ...
            if let wVal = wScanned, let hVal = hScanned {
                ...
                h = hVal
                 ...
                 throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' into CGSize. Expected format like 'w=100,h=50' or '100,50'.")
            }
        }
        ...
        if components.count == 4,
           ...
           let hStr = components[3].split(separator: "=").last, let hVal = Double(hStr) {
            ...
            if let xS = xS_opt, let yS = yS_opt, let wS = wS_opt, let hS = hS_opt {
                ...
                throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' into CGRect. Expected format like 'x=0,y=0,w=100,h=50' or '0,0,100,50'.")
            }
        }
        ...
        if components.count == 2,
           ...
           let lenStr = components[1].split(separator: "=").last, let lenVal = Int(lenStr) {
            ...
            if let locV = locScanned, let lenV = lenScanned {
                ...
                throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' into CFRange. Expected format like 'loc=0,len=10' or '0,10'.")
            }
        }
    ...
    case .illegal:
        throw AXToolError.attributeUnsupported("Cannot parse value for AXValueType .illegal")
        
    case .axError: // Should not be settable
         throw AXToolError.attributeUnsupported("Cannot set an attribute of AXValueType .axError")
    ...
    default:
        ...
        if targetAXValueType.rawValue == 4 { // Empirically, AXValueBooleanType is 4
            ...
            if stringValue.lowercased() == "true" {
                ...
                throw AXToolError.valueParsingFailed(details: "Could not parse '\(stringValue)' as boolean for AXValue.")
            }
            valueRef = AXValueCreate(targetAXValueType, &boolVal)
        } else {
            throw AXToolError.attributeUnsupported("Parsing for AXValueType '\(stringFromAXValueType(targetAXValueType))' (rawValue: \(targetAXValueType.rawValue)) from string is not supported yet.")
        }
    }

    if valueRef == nil {
         throw AXToolError.valueParsingFailed(details: "AXValueCreate failed for type \(stringFromAXValueType(targetAXValueType)) with input '\(stringValue)'")
    }
    return valueRef
} 