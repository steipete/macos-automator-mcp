    ...
    for (const source of sources) {
      ...
      if (typeof source === "string") {
        ...
        try {
          ...
          const fileExists = app
            .doShellScript(`test -f "${source}" && echo "exists" || echo "not found"`)
            .trim();
    ...
    for (const key in source) {
      if (Array.isArray(source[key])) {
        ...
        target[key] = Array.from(
          new Set(merged.map((item) => (typeof item === "object" ? JSON.stringify(item) : item))),
        ).map((item) =>
...
{
  "action": "mergeJSON",
  "sources": ["/path/to/first.json", "/path/to/second.json", { "additionalData": "inline data" }],
  "strategy": "deep"
}
```
