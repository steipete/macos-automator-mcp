            ...
            if (target && Array.isArray(target[lastField])) {
              if (transform.key) {
                ...
                target[lastField].sort((a, b) => {
                  const valueA =
                    typeof a[transform.key] === "string"
                      ? a[transform.key].toLowerCase()
                      : a[transform.key];
                  const valueB =
                    typeof b[transform.key] === "string"
                      ? b[transform.key].toLowerCase()
                      : b[transform.key];
              ...
              for (const agg of transform.aggregates) {
                if (agg.type === "sum" && agg.property) {
                  aggregateResults[`sum_${agg.property}`] = target[lastField].reduce(
                    (sum, item) => sum + (parseFloat(item[agg.property]) || 0),
                    0,
                  );
                  ...
                  aggregateResults[`min_${agg.property}`] = Math.min(
                    ...target[lastField].map((item) => parseFloat(item[agg.property]) || 0),
                  );
                } else if (agg.type === "max" && agg.property) {
                  aggregateResults[`max_${agg.property}`] = Math.max(
                    ...target[lastField].map((item) => parseFloat(item[agg.property]) || 0),
                  );
