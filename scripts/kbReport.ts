...
export function logErrorToReport(
  filePath: string,
  message: string,
  isLocalKbIssue = false,
  currentReport: ValidationReport = report,
) {
  const prefix = isLocalKbIssue ? "LOCAL_KB " : "";
  currentReport.errors.push(`ERROR: ${prefix}[${filePath}] ${message}`);
}

export function logWarningToReport(
  filePath: string,
  message: string,
  isLocalKbIssue = false,
  currentReport: ValidationReport = report,
...
export function printValidationReport(currentReport: ValidationReport = report) {
  ...
  console.log(
    `Categories Found: ${currentReport.categoriesFound.size} (${Array.from(currentReport.categoriesFound).sort().join(", ")})`,
  );
  ...
  if (currentReport.fixesApplied.length > 0) {
    ...
    console.log(
      "Specific auto-fixes were logged during processing (currently disabled for file writes).",
    );
  } else {
  ...
  if (currentReport.duplicateTipIdsPrimary.length > 0) {
    ...
    console.warn(
      "The following Tip IDs were duplicated in the primary KB. Ensure each explicit 'id' in frontmatter is unique, and filenames within categories also lead to unique generated IDs:",
    );
  ...
  if (currentReport.duplicateSharedHandlersPrimary.length > 0) {
    ...
    console.warn(
      "The following Shared Handler names (name_language) were duplicated in the primary KB. Ensure unique names:",
    );
  ...
  if (currentReport.overriddenSharedHandlersLocal.length > 0) {
    ...
    console.info(
      "The following Shared Handlers from the primary KB were overridden by a local version:",
    );
  ...
  if (
    ...
    currentReport.duplicateTipIdsPrimary.length > 0 ||
    currentReport.duplicateSharedHandlersPrimary.length > 0
  ) {
    process.exitCode = 1;
  }
}
