...
export const ExecuteScriptInputSchema = z
  .object({
    kb_script_id: z
      .string()
      .optional()
    ...
    include_executed_script_in_output: z
      .boolean()
      .optional()
      .default(false)
      .describe(
        "If true, the executed script content (after substitutions) or path will be included in the output.",
      ),
    include_substitution_logs: z
      .boolean()
      .optional()
      .default(false)
      .describe(
        "If true, detailed logs of placeholder substitutions will be included in the output.",
      ),
  })
  .refine(
    (data) => {
      const sources = [data.script_content, data.script_path, data.kb_script_id].filter(
    ...
    {
      message:
        "Exactly one of 'script_content', 'script_path', or 'kb_script_id' must be provided and be non-empty.",
...
export const GetScriptingTipsInputSchema = z.object({
  ...
  list_categories: z
    ...
    .default(false)
    .describe(
      "If true, returns only the list of available categories and their descriptions. Overrides other parameters.",
    ),
  refresh_database: z
    .boolean()
    .optional()
    .describe("If true, forces a reload of the knowledge base before processing the request."),
  limit: z
    .number()
    .int()
    .positive()
